...
public class CarController : MonoBehaviour {
	...
	//GameObject PlayerInCar;
	public SoundController sc;
	Text vehiclSpeedText;
	...
	void Start () 
	{
		vehiclSpeedText = CanvasManager.instance.vehiclSpeedText;
		
		if (centerOfMass != null)
			rigid.centerOfMass = centerOfMass.localPosition;
		rigid.inertiaTensor *= inertiaFactor;
	...
	{	
		if(controlsEnabled){
			speed = rigid.linearVelocity.magnitude * 2.0f;
			vehiclSpeedText.text = speed.ToString("F0");
		}
		
		// Steering
		Vector3 carDir = transform.forward;
		float fVelo = rigid.linearVelocity.magnitude;
		Vector3 veloDir = rigid.linearVelocity * (1/fVelo);
		...
		bool accelKey = Input.GetKey (KeyCode.W);
		...
		if (drivetrain.automatic && drivetrain.gear == 0)
		{
			if(controlsEnabled)
			{
				accelKey = Input.GetKey (KeyCode.S);
				brakeKey = Input.GetKey (KeyCode.W);
			}
		}
		...
		{
			if (drivetrain.slipRatio < 0.10f && controlsEnabled)
				throttle += Time.deltaTime / throttleTime;
			else if (!tractionControl && controlsEnabled)
		...
		if(Input.GetKeyDown(KeyCode.X) && controlsEnabled)
		{
			lastShiftTime = Time.time;
			drivetrain.ShiftUp ();
		}
		if(Input.GetKeyDown(KeyCode.Z) && controlsEnabled)
		{
			lastShiftTime = Time.time;
	...
	public void Status(bool s){
		controlsEnabled = s;
		
		if(controlsEnabled){
			sc.playerIn = true;
			//if(PlayerInCar) PlayerInCar.SetActive(true);
		}else{
			sc.playerIn = false;
			//if(PlayerInCar) PlayerInCar.SetActive(false);
		}
	}
}